...
# Add firewall rule
az sql server firewall-rule create -g RG-TEST -s how-will-calven-trigger-okta -n allowLocalMachineRule --start-ip-address ${START_IP_ADDRESS} --end-ip-address ${END_IP_ADDRESS} &> /dev/null
...
az sql server firewall-rule delete -g RG-TEST -s how-will-calven-trigger-okta -n allowLocalMachineRule;
